...
Ollama는 단일 모델을 로드할 때 다음 **우선순위**로 메모리를 할당합니다:

1. **① 단일 GPU에 전부 적재** — 모델이 하나의 GPU VRAM에 들어가면 해당 GPU에 전부 올립니다. 이것이 PCI 버스를 타지 않으므로 **가장 빠릅니다.**
2. **② 여러 GPU에 레이어 분산** — 하나의 GPU에 들어가지 않으면, **나머지 레이어를 다른 GPU에 순차적으로 분배**합니다. (Pipeline Parallelism)
3. **③ CPU RAM으로 오프로딩** — 모든 GPU VRAM을 사용해도 부족하면, 나머지를 **CPU RAM(시스템 메모리)으로 오프로딩**합니다. (가장 느림)

즉, **CPU RAM 오프로딩은 최후의 수단**이며, 사용 가능한 GPU가 있다면 먼저 GPU에 분산합니다.

> **❓ 왜 CPU로 바로 안 가고 다른 GPU에 분산하나요?**
> GPU VRAM은 CPU RAM보다 대역폭이 수십 배 빠릅니다 (예: H200 HBM3e ~4.8TB/s vs DDR5 ~100GB/s). 따라서 다른 GPU에 분산하는 것이 CPU로 오프로딩하는 것보다 훨씬 유리합니다.

**여러 모델 동시 로딩:**

`OLLAMA_MAX_LOADED_MODELS=N`으로 설정하면 N개의 모델을 동시에 VRAM에 올릴 수 있습니다. 이 경우 각 모델은 **가능한 한 서로 다른 GPU에 분산 배치**됩니다.

```
# H200 8장 예시 - 3개 모델 동시 로딩
GPU 0: [모델 A 전체 레이어]         ← 1 GPU에 들어가는 작은 모델
GPU 1: [모델 B 전체 레이어]         ← 1 GPU에 들어가는 작은 모델
GPU 2-3: [모델 C 레이어 분산]       ← 큰 모델은 여러 GPU에 걸쳐 분배
GPU 4-7: [유휴]

# 만약 초대형 모델(예: 70B) 하나만 올린다면?
GPU 0-5: [모델 D 레이어 분산]       ← 6개 GPU에 걸쳐 분배 (Pipeline)
GPU 6-7: [유휴]
CPU RAM: [일부 레이어]              ← GPU에 다 안 들어가면 CPU로 오프로딩
```

> **⚠️ TP(Tensor Parallelism) vs PP(Pipeline Parallelism) — 왜 Ollama는 PP인가?**
>
> | 구분 | Tensor Parallelism (vLLM) | Pipeline Parallelism (Ollama) |
> | :--- | :--- | :--- |
> | **동작 방식** | 하나의 레이어를 여러 GPU에 **쪼개서 동시 연산** | 각 GPU가 **서로 다른 레이어를 순차 처리** |
> | **속도 효과** | GPU 수에 비례하여 **단일 추론 속도 가속** | **용량은 늘지만, 속도는 거의 안 빨라짐** |
> | **GPU 간 통신** | 매우 빈번 (all-reduce) | 최소한 (레이어 경계에서만) |
> | **구현 복잡도** | 높음 (NCCL 등 필요) | 낮음 |
>
> Ollama의 백엔드인 **llama.cpp**는 경량화와 이식성을 최우선으로 설계되었기 때문에, 구현이 복잡하고 GPU 간 고속 통신 인프라(NVLink 등)를 요구하는 Tensor Parallelism 대신, 단순하게 레이어를 나누어 배치하는 Pipeline Parallelism을 채택했습니다.
>
> **실질적 의미**: H200 8장에 70B 모델을 올려도, Ollama에서는 추론 속도가 단일 GPU 때와 비슷합니다. (메모리만 확장) 반면 vLLM에서는 `-tp 8`로 설정하면 추론 속도가 8배 가까이 빨라집니다.
