...
조금 더 정확히 말하면, LLM은 지금까지 나온 문맥을 보고 다음에 올 가능성이 높은 토큰을 하나씩 고르는 방식으로 글을 생성합니다.  
...
LLM은 우리가 입력한 문장을 사람처럼 문장이나 단어 그대로 읽지 않습니다.  
모델이 처리하는 더 작은 단위로 문장을 쪼갠 뒤, 그 조각들을 숫자로 바꿔서 처리합니다.
...
그래서 앞으로 `다음 단어를 예측한다`고 말하더라도, 실제 내부 동작은 더 정확히 다음 토큰을 예측하는 데 가깝습니다.
...
이 차이는 서빙 최적화에서도 중요한데요.  
모델의 입력 길이, 출력 길이, 처리 속도, 비용, 메모리 사용량은 대부분 문장 수가 아니라 **토큰 수**를 기준으로 계산되기 때문입니다.

## 3. 다음 토큰을 예측한다는 것은 무엇인가요?

LLM이 글을 생성할 때 하는 핵심 작업을 단순하게 말하면 다음과 같습니다.

```text
지금까지의 토큰들을 보고, 다음 토큰 후보들의 점수를 계산합니다.
```

예를 들어 현재 문맥이 다음과 같다고 해볼까요?

```text
오늘 저녁에는
```

모델은 이 문맥 뒤에 올 토큰 후보마다 점수를 계산합니다.  
...
설정에 따라 가장 높은 후보를 고르거나, 어느 정도 무작위성을 섞어 다른 후보를 고를 수 있습니다.
...
LLM이 토큰을 하나씩 생성한다는 점을 이해했다면, 이제 실제 추론 흐름을 조금 더 정확히 나눠보겠습니다.  
...
예를 들어 사용자가 `오늘 저녁에는`이라는 프롬프트를 보냈다면, 모델은 이 입력 토큰들을 처리하면서 이 문맥 다음에 어떤 토큰이 올 수 있는지를 계산할 준비를 합니다.

여기서 입력 토큰들을 한 번에 처리한다는 말은, 답변을 생성할 때처럼 `치킨 -> 을 -> 먹고`를 순서대로 하나씩 뽑는다는 뜻이 아닙니다.  
...
즉 Prefill은 사용자의 입력을 읽고 첫 출력 토큰을 만들기 전까지의 준비 작업에 가깝습니다.
...
그래서 LLM 서빙에서는 단순히 모델이 얼마나 똑똑한지만 보지 않습니다.  
다음과 같은 질문을 함께 봅니다.
...
다음 토큰을 예측할 때마다 모델은 지금까지의 문맥을 참고해야 합니다.  
그렇다면 이런 의문이 생깁니다.
...
그래서 LLM 서빙에서는 이전 토큰을 계산하며 얻은 일부 중간 결과를 저장해 두고 재사용합니다.  
이때 사용하는 대표적인 기술이 **KV Cache**입니다.

KV Cache는 쉽게 말해, 모델이 이전 문맥을 처리하면서 만들어 둔 계산 메모를 저장해 두는 방식입니다.  
...
즉 KV Cache를 사용하면 LLM 서빙에서 아주 중요한 상충 관계가 생깁니다.

```text
계산을 아끼기 위해 메모리를 쓴다.
```

이 상충 관계를 파악하면 이후에 등장하는 `PagedAttention`, `Prefix Caching`, `FP8 KV Cache`, `Chunked Prefill` 같은 최적화 기법도 훨씬 자연스럽게 이해됩니다.
