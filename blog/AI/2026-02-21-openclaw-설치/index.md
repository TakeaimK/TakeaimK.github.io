...
authors: [me]
tags: [ai, openclaw, hyper-v, vm, gemini, ollama, slack, discord]
---
...
이번 글에서는 **윈도우 PC에서 Hyper-V를 이용해 우분투 가상 머신(VM)을 만들고**, 그 위에 OpenClaw를 설치하여 AI 모델을 연동하는 전 과정을 다뤄봅니다. 추가로 **Slack, Discord** 같은 메신저 채널도 연결해 봅니다.
...
Slack, Discord 등 메신저 채널을 연동할 수 있습니다. 지금 당장 설정하지 않으려면 방향키로 `Skip for now`를 선택하고 넘어가도 괜찮습니다. (채널 연동은 글 *6번 목차*에서 따로 상세히 다룹니다)
...
OpenClaw의 진정한 힘은 **메신저 채널과 연동**할 때 빛납니다. 브라우저 UI 외에도 Slack, Discord 같은 일상적인 메신저에서 AI 비서와 대화할 수 있습니다.

### 6.1 Slack 연동

Slack 워크스페이스에 OpenClaw 봇을 추가하여 대화하는 방법입니다. **Socket Mode**를 사용하므로 공인 IP나 포트포워딩이 필요 없습니다.

#### Step 1. Slack App 생성

1. [api.slack.com/apps](https://api.slack.com/apps)에 접속해서 로그인합니다.
2. **Create New App** → **From scratch** 선택
3. **App Name**: 원하는 이름 입력 (예: `OpenClaw`)
4. **Workspace**: 연결할 워크스페이스 선택 → **Create App**

#### Step 2. Socket Mode 활성화 + App Token 생성

1. 왼쪽 사이드바에서 **Socket Mode** 클릭
2. **Enable Socket Mode** 토글 ON
3. App-Level Token 생성 팝업이 뜨면:
   - **Token Name**: 아무 이름 (예: `openclaw-socket`)
   - **Scope**: `connections:write` 추가
   - **Generate** 클릭
4. 생성된 `xapp-` 으로 시작하는 토큰을 **복사해서 메모**해 둡니다. 이것이 **App Token**입니다.

#### Step 3. Bot Token Scopes 설정

1. 왼쪽 사이드바에서 **OAuth & Permissions** 클릭
2. **Bot Token Scopes** 섹션까지 스크롤
3. **Add an OAuth Scope**을 클릭해서 아래 스코프를 **모두 추가**합니다:
...
| `assistant:write` | 타이핑 인디케이터 + 스트리밍 |
| `reactions:read` | 리액션 읽기 |
| `reactions:write` | 리액션 추가 |
| `pins:read` | 핀 읽기 |
| `pins:write` | 핀 추가 |
| `emoji:read` | 이모지 목록 조회 |
| `commands` | 슬래시 명령어 |
| `files:read` | 파일 읽기 |
| `files:write` | 파일 업로드 |

#### Step 4. 앱 워크스페이스에 설치 + Bot Token 복사

1. **OAuth & Permissions** 페이지 상단의 **Install to Workspace** 클릭
2. 권한 요청 화면에서 **Allow** 클릭
3. 설치 완료 후 나타나는 **Bot User OAuth Token** (`xoxb-`로 시작)을 **복사해서 메모**합니다.

#### Step 5. Event Subscriptions 설정

1. 왼쪽 사이드바에서 **Event Subscriptions** 클릭
2. **Enable Events** 토글 ON
3. **Subscribe to bot events** 섹션에서 **Add Bot User Event**를 클릭해서 아래 이벤트를 모두 추가:
   - `app_mention`
   - `message.channels`
   - `message.groups`
   - `message.im`
...
1. 왼쪽 사이드바에서 **App Home** 클릭
2. **Show Tabs** 섹션에서 **Messages Tab** 체크 활성화
3. **"Allow users to send Slash commands and messages from the messages tab"** 도 체크

> **Warning**: 이 설정이 없으면 DM이 작동하지 않습니다.

#### Step 7. (선택) Slash Command 등록

슬래시 명령어를 사용하려면:

1. 왼쪽 사이드바에서 **Slash Commands** 클릭
2. **Create New Command**:
   - **Command**: `/openclaw`
   - **Short Description**: `Send a message to OpenClaw`
   - **Usage Hint**: (비워두거나 자유 입력)
3. Socket Mode에서는 **Request URL 입력 불필요** (자동 처리)
4. **Save** 클릭

#### Step 8. (선택) Agents & AI Apps 활성화

Slack 네이티브 텍스트 스트리밍을 사용하려면:

1. 왼쪽 사이드바에서 **Agents & AI Apps** 클릭
2. 해당 기능을 **활성화**

> **Tip**: 이 기능을 켜면 OpenClaw 응답이 Slack 스레드에서 실시간 타이핑 스트리밍으로 표시됩니다. Step 3에서 추가한 `assistant:write` 스코프가 필요합니다.

#### Step 9. OpenClaw에 Slack 채널 설정

`~/.openclaw/openclaw.json` 설정 파일을 열고 `channels` 구문에 `slack`을 추가합니다.
...
// ~/.openclaw/openclaw.json
{
  channels: {
    slack: {
      enabled: true,
      mode: "socket",
      appToken: "xapp-여기에-Step2에서-복사한-App-Token",
      botToken: "xoxb-여기에-Step4에서-복사한-Bot-Token",
      dmPolicy: "pairing",        // 첫 DM 시 페어링 승인 필요 (혼자 쓸 거면 "open"으로 변경 가능)
      groupPolicy: "open",        // 모든 채널에서 봇 응답 가능 (특정 채널만 허용하려면 "allowlist")
      streaming: true,            // Slack 네이티브 스트리밍 활성화 (Step 8 필요)
      textChunkLimit: 4000,       // 한 메시지의 최대 글자수 (가독성 기본값)
      chunkMode: "newline",       // 긴 텍스트를 문단 경계에서 분할
    },
  },
}
```

#### Step 10. Gateway 재시작 및 페어링
...
# Slack 채널 연결 상태 로그 확인
openclaw logs --follow
```

이후 Slack 워크스페이스에서:

1. 왼쪽 사이드바 **Apps** 섹션에서 OpenClaw 봇을 찾습니다.
2. 봇을 클릭해서 DM을 엽니다.
3. 아무 메시지를 보내면 **페어링 코드**가 나타납니다 (예: `Your pairing code is: ABC123`).
4. 터미널에서 페어링을 승인합니다:
...
5. 다시 Slack DM에서 메시지를 보내면 OpenClaw이 응답합니다.

#### Step 11. (선택) 채널에서 사용하기

퍼블릭 채널에서 봇을 사용하려면:

1. 원하는 Slack 채널에 `/invite @OpenClaw` 입력해서 봇을 초대합니다.
2. `@OpenClaw 안녕하세요` 처럼 **@멘션**해서 말을 겁니다.

채널에서는 기본적으로 @멘션이 있어야만 응답합니다 (노이즈 방지). 특정 채널에서 멘션 없이도 응답하게 하려면:
...
> 📎 [Slack 연동 공식 문서](https://docs.openclaw.ai/channels/slack)
...
| 비교 항목 | Slack | Discord |
| :--- | :--- | :--- |
| **토큰 발급처** | Slack API Console | Discord Developer Portal |
| **연결 방식** | Socket Mode (공인 IP 불필요) | Bot Token (공인 IP 불필요) |
| **플러그인 필요** | ❌ 불필요 | ❌ 불필요 |
| **HTTPS 필수** | ❌ | ❌ |
| **스트리밍 지원** | ✅ 네이티브 스트리밍 | ✅ |
| **그룹 채팅 지원** | ✅ | ✅ |
